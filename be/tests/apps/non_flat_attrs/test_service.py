...
class TestNonFlatAttrsService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_tree_node_and_property_exist(self):
        self.mock_repository.get_latest_node_id = AsyncMock(return_value=(1, 2, 3))
        self.mock_repository.get_property_by_latest_node_id = AsyncMock(
            return_value=NonFlatAttrsPropertySchema(
                name="Test node", value=Decimal(30.155), created_at=datetime.now()
            )
        )
        self.mock_repository.get_node_list_by_node_id = AsyncMock(return_value=[])
        self.mock_repository.factory.serialize = MagicMock(
            return_value=NonFlatAttrsNodeSchema(
                id=1, name="Test node", created_at=datetime.now()
            )
        )
    ...
    async def test_create_node_already_exists(self):
        self.mock_repository.get_latest_node_id = AsyncMock(return_value=(1, 0, 0))
        with self.assertRaises(NonFlatAttrsException):
            await self.service.create("route_path", None)

    async def test_create_successful(self):
        self.mock_repository.get_latest_node_id = AsyncMock(return_value=(1, 1, 1))
    ...
    async def test_delete_node_successful(self):
        ...
        self.mock_repository.delete_property.assert_called_once_with(
            property_id_list=[6, 7]
        )
        self.mock_repository.delete_node.assert_called_once_with(
            node_id_list=[3, 10, 11, 12, 13, 14, 15]
        )
